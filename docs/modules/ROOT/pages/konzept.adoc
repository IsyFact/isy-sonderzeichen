...
Im Konzept werden zunächst die Anforderungen aufgeführt, die an die Verarbeitung von Sonderzeichen innerhalb von auf IsyFact-Standards basierenden Anwendungen gestellt werden.
...
Internationalisierung ist nicht Bestandteil dieses Konzepts.
...
Für nach xref:glossary::terms-definitions.adoc#glossar-ifs[IsyFact-Standards] entwickelte Anwendungen ist der zu verwendende Zeichensatz festgelegt.
...
Die Umwandlung kann durch Transkription geschehen.
 +
*Beispiel:* Ein Nachbarsystem arbeitet ausschließlich mit dem ASCII-Zeichensatz.
...
Dadurch verbessert sich aber nicht unbedingt die Trefferqualität.
 +
*Beispiel:* "Müller" wird im Originalschreibweise gespeichert und für die Suche zu "Mueller" umgeschrieben.
Eine Suchanfrage nach "Müller" wird zunächst zu "Mueller" umgeschrieben, dann gesucht und auch gefunden.
Eine Suchanfrage nach "Mueller" wird ohne Transkription direkt gefunden.
...
In der xref:nutzungsvorgaben/tabellen.adoc#table-transkription[Tabelle Transkription] sind die Transkriptionsregeln für die verschiedenen Zeichen dargestellt.
...
[[sonderzeichen_klasse_transtext]]
...
Die Transkription ist als Komponente umgesetzt, die bei Bedarf in ein IT-System eingebunden wird.
...
[[sonderzeichen_transkription.png]]
.Komponente Transkription
image::konzept/sonderzeichen_transkription.png[width=50%,align="center"]
...
Die Standard-Zeichencodierung kann als Aufrufparameter in der JVM gesetzt werden (siehe auch Kapitel xref:nutzungsvorgaben.adoc#java[Java] in den Nutzungsvorgaben).
